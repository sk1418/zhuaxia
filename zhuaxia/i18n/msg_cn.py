# -*- coding:utf-8 -*-
fmt_init_song_xm=u'[虾]开始初始化歌曲[%s]'
fmt_err_song_parse_xm=u'[虾]无法解析/下载歌曲链接: [%s]'
fmt_init_song_ok_xm=u'[虾]初始化歌曲成功[%s]'
fmt_init_album_xm=u'[虾]开始初始化专辑[%s]'
fmt_create_album_dir_xm=u'[虾]创建专辑目录[%s]' 
fmt_dl_album_cover_xm=u'[虾]下载专辑[%s]封面'
fmt_save_album_desc_xm=u'[虾]保存专辑[%s]介绍'
fmt_init_fav_xm=u'[虾]开始初始化用户收藏[%s]'
fmt_parse_song_url_xm =u'[虾]解析歌曲链接[%s]'
fmt_init_fav_ok_xm=u'[虾]初始化用户收藏完毕[%s]'
fmt_init_collect_xm=u'[虾]开始初始化精选集[%s]'
fmt_init_collect_ok_xm=u'[虾]初始化精选集完毕[%s]'
fmt_init_artist_xm=u'[虾]初始化艺人TopSong[%s]'
fmt_init_artist_ok_xm=u'[虾]初始化艺人TopSong完毕[%s]'
dl_128kbps_xm=u'[虾] 不登录虾米进行下载, 虾米资源质量为128kbps.'
fmt_login_ok_xm=u'[Login] 用户: %s (id:%s) 登录成功.' 
login_xm=u'[虾] 登录虾米...'
login_err_xm=u'[虾] 登录失败, 略过登录, 虾米资源质量为 128kbps.'
